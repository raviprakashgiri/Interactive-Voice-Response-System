...
<c:choose>
    ...
    audioResponse.addGotoNEXTURL(ConfigParams.getIvrslink()+"RepeatBroadcastMessage_UnRegistered.jsp?groups_id="+groups_id+"&caller_id="+caller_id+"&register_number="+register_number); 
	...
	<c:when
		...
		<%
        	try{
        				...
        				String registered_number = request.getParameter("register-number");

        				//interpreting the selected option
        				if (language.equals(null) || language.equals("")
        						|| language == "" || language == null) {

        					kResponse.addGotoNEXTURL(ConfigParams.getIvrslink()
        									+ "MenuLanguage_UnRegistered.jsp?event=NewCall&groups_id="
        									+ Groups_id + "&register_number="
        									+ registered_number);
        					...
        					try {
        						String values[] = call
        								.getGroupsIdBroadcastMessageFromCallerId(
        										callerID).split(",");
        						...
        						if (broadcastMessage
        								.equals("No broadcast message")) {
        							kResponse.addPlayText(broadcastMessage);
        							kResponse
        									.addGotoNEXTURL(ConfigParams
        											.getIvrslink()
        											+ "MenuLanguage_UnRegistered.jsp?event=NewCall&groups_id="
        											+ Groups_id
        											+ "&register_number="
        					...
        					kResponse
        							.addGotoNEXTURL(ConfigParams.getIvrslink()
        									+ "RepeatBroadcastMessage_UnRegistered.jsp?event=NewCall&groups_id="
        									+ Groups_id + "&caller_id="
        									+ callerID + "&register_number="
        									+ registered_number);
        				}
